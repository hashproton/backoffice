<script lang="ts">
	...
	$effect(() => {
		...
		if (isApiError(data) || data.errors) {
			setTimeout(() => {
		...
		filterData.name = query.get('name') || '';
		filterData.status = query.get('status') || 'all';
	});
	...
	>
		<div class="grid auto-rows-max items-start gap-4 md:gap-8 lg:col-span-2">
			<div class="flex justify-end gap-2">
				<Input bind:value={filterData.name} name="name" placeholder="Search tenants" onkeydown={handleFilteredName} />
							...
							<Pagination.Content>
								<Pagination.Item>
									<Button
										...
										onclick={() => gotoQs('page', data.pageNumber! - 1)}
									>
								...
								{#each pages as page (page.key)}
									...
									{:else}
										<Pagination.Item onclick={() => gotoQs('page', page.value)}>
